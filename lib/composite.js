...
const ObjectElement = Element.extend('Object', createConstructedByProperties(Object), {
    ...
    clone() {
        const clone = this.procreate(this.generate(this.value));
        return clone;
    },
    ...
    infect(methods) {
        const infection = {
            methods: methods,
            propagate(element) {
                const propagatedInfection = Object.create(this);

                propagatedInfection.element = element;
                propagatedInfection.infect();
            },
            ...
            infect() {
                ...
                Object.keys(this.methods).forEach(function(key) {
                    if (element.hasOwnProperty(key)) {
                        uninfected[key] = element[key];
                    }
                    element[key] = this.methods[key];
                }, this);
                this.uninfected = uninfected;
                if (element.hasOwnProperty('infection')) {
                    this.previous = element.infection;
                }
                element.infection = this;
            },

            cure() {
                const element = this.element;
                const uninfected = this.uninfected;
                Object.keys(this.methods).forEach(function(key) {
                    if (uninfected.hasOwnProperty(key)) {
                        element[key] = uninfected[key];
                    } else {
                        delete element[key];
                    }
                }, this);
                ...
                if (previous) {
                    element.infection = previous;
                } else {
                    delete element.infection;
                }
            },
            ...
            purify() {
                ...
                while (infection) {
                    infection.cure();
                    infection = infection.previous;
                }
            }
        };
        infection.propagate(this);
        return this;
    },

    cure() {
        const infection = this.infection;
        if (infection) {
            infection.cure();
        }
        return this;
...
const CombineObjectReaction = Reaction.extend({
    produce(firstObject, secondObject) {
        const compositeObject = this.produceComposite(firstObject, secondObject);
    ...
    produceComposite(firstObject, secondObject) {
        ...
        const compositeObject = firstObject.procreate(combinedValue);
...
Element.refine({
    procreate(value) {
        const progeny = this.createConstructor(value);
        const infection = this.infection;
        if (infection) {
            infection.propagate(progeny);
        }
        return progeny;
    },
    ...
    transformation: createDynamicTransformation(
        ...
        CopyTransformation.extend({
            produce(arrayProperty, parentElement) {
                const product = CopyTransformation.produce.apply(this, arguments);
