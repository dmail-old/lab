...
const ObjectElement = Element.extend('Object', createConstructedByProperties(Object), {
    ...
    compile() {
        ...
        const instantiationInfection = Infection.create({
            ...
            // il faudrais modifier le clone packager pour ne pas set les primitifs
            // pour cela il faudrais pouvoir lire chaque propriété, et celle étant des valeurs
            // si leur valeur est une primitive ne pas set
            // ok facile MAIS cette logique est dans transformation ou reaction
            // est ce qu'on pourrait pas juste avoir le quatuor make/insert/fill/compile|pack
            // et ce quatuor selon si on est en mode clone ou copy
            // ne fait pas la même chose
            // comme ça on peut infecter facilement
            // en gros lorsqu'un élément réagit avec un autre il se retrouve infecté par les méthodes de réaction qui lui son propre
            // alors pourquoi pas mais comment faire prévaloir ma méthode dans certain cas, la méthode par défaut dans d'autre ?
            // dans la même idée les objet pourrait être porteur sain de plusieurs infections genre comment réagir en cas
            // de composition et cette infection se déclencherait lorsqu'on compose
            // on pourrait override ou modifier l'infection pour qu'elle se comporte différement
            // on aurait ainsi le comportement voulu
            // c'est à dire un comportement général qui reste cusomtizable et héritable
            // transformation et reaction disparaitrais au profil d'infection qui se déclenche au moment choisit
            // puis on redeviendrais porteur sain une fois l'opération terminé
            // à réfléchir intensément pour voir est ce que ça match bien
            // on aurais besoin d'une sorte de dynamicInfection qui selon comment l'infection est déclaré
            // elle se manifeste différent (pendent de dynamicReaction qui réagit différement selon ce qui est composé)
        });
...
const CombineObjectReaction = Reaction.extend({
    maker(firstObject, secondObject) {
        const compositeObject = firstObject.createComposite(firstObject, secondObject);
...
ObjectElement.refine({
    ...
    reaction: createDynamicReaction(
        function(firstElement, secondElement) {
            return secondElement.primitiveMark;
        },
        VanishReaction,
        CombineObjectReaction
    ),
    createComposite(firstObject) {
        return firstObject.procreate();
    },
    fillComposite() {
        this.value = {};
    },
...
ArrayPropertyElement.refine({
    ...
    transformation: createDynamicTransformation(
        ...
        CopyTransformation.extend({
            filler(arrayPropertyCopy, arrayProperty, parentElement) {
                CopyTransformation.filler.apply(this, arguments);
                ...
                if (propertyTrackingEntries) {
                    ...
                    arrayPropertyCopy.name = concatenedIndexAsString;
                    if (debugArrayConcat) {
    ...
    reaction: createDynamicReaction(
        ...
        CombinePropertyReaction.extend({
            handleConstituantReaction(compositeLength, firstLength) {
                ...
                resetCompositeLengthTransformation.fillClone = function() {
                    this.value = 0;
                };
