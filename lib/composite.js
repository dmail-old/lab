...
const CombineObjectReaction = Reaction.extend({
    produce(firstObject, secondObject) {
        ...
        const comparison = Lab.compareSpecificity(firstObject, secondObject);
        let mostSpecificElement;
        if (comparison < 0) {
            mostSpecificElement = secondObject;
        } else {
            mostSpecificElement = firstObject;
        }
        const combinedValue = mostSpecificElement.combine(firstValue, secondValue);
        const compositeObject = mostSpecificElement.createConstructor(combinedValue);
    ...
    reaction: createDynamicReaction(
        ...
        CombinePropertyReaction.extend({
            handleConstituantReaction(compositeLength, firstLength) {
                ...
                const resetCompositeLengthTransformation = CopyTransformation.create(firstLength, compositeLength);
                resetCompositeLengthTransformation.refine = function(compositeLengthCopy, compositeLength) {
                    const resetValueTransformation = CopyTransformation.create(
